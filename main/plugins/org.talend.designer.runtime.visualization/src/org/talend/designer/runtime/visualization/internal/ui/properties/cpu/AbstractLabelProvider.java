 ...
 */
abstract public class AbstractLabelProvider implements ITableLabelProvider, ILabelProvider, ITableFontProvider, ISharedImages {
    ...
    private static final String[] PERCENTAGE_IMG_PATHS = { PERCENT_0_IMG_PATH, PERCENT_2_IMG_PATH, PERCENT_4_IMG_PATH,
            PERCENT_7_IMG_PATH, PERCENT_10_IMG_PATH, PERCENT_15_IMG_PATH, PERCENT_20_IMG_PATH, PERCENT_25_IMG_PATH,
            PERCENT_30_IMG_PATH, PERCENT_35_IMG_PATH, PERCENT_40_IMG_PATH, PERCENT_50_IMG_PATH, PERCENT_60_IMG_PATH,
            PERCENT_70_IMG_PATH, PERCENT_80_IMG_PATH, PERCENT_90_IMG_PATH, PERCENT_100_IMG_PATH };

    /** The percentage categories. */
    private static final double[] percentageCategories = { 1, 3, 5.5, 8.5, 12.5, 17.5, 22.5, 27.5, 32.5, 37.5, 45, 55, 65, 75,
            85, 95 };
    ...
    protected Image getThreadImage() {
        if (threadImage == null || threadImage.isDisposed()) {
            threadImage = Activator.getImageDescriptor(THREAD_IMG_PATH).createImage();
        }
        return threadImage;
    }
    ...
    protected Image getMethodImage() {
        if (methodImage == null || methodImage.isDisposed()) {
            methodImage = Activator.getImageDescriptor(METHOD_IMG_PATH).createImage();
        }
        return methodImage;
    }
    ...
    protected Image getPercentageImage(double percentage) {
        ...
        if (perrcentageImages[index] == null || perrcentageImages[index].isDisposed()) {
            perrcentageImages[index] = Activator.getImageDescriptor(PERCENTAGE_IMG_PATHS[index]).createImage();
        }
    ...
    protected String getMillisecondsText(long milliseconds, int length) {
        return String.format(Messages.percentageLabel + length + "d" + Messages.millisecondsLabel, //$NON-NLS-1$
