...
public class InvokeDialog extends Dialog {
    ...
    protected void configureShell(Shell newShell) {
        ...
        PlatformUI.getWorkbench().getHelpSystem().setHelp(newShell, IHelpContextIds.INVOKE_DIALOG);
    }
     ...
     * 
     * @param arrayType The type (e.g. [J)
     * @param string The string (e.g. 1,2,3)
     * @return True if the given number is valid
     */
        ...
        new Job(Messages.invokeMBeanOperationJobLabel) {
            ...
            protected IStatus run(IProgressMonitor monitor) {
                try {
                    ...
                    setMessage(NLS.bind(Messages.mBeanOperationFailedLabel, new Date()));
                ...
                setMessage(NLS.bind(Messages.mBeanOperationSucceededLabel, new Date()));
    ...
    void doInvoke() throws JMException, IOException {
        ...
        final Object result;
        try {
            result = jvm.getMBeanServer().invoke(objectName, operationName, params, signature);
        ...
        Display.getDefault().asyncExec(new Runnable() {
            ...
            public void run() {
                ...
                if (attributeNode.hasChildren()) {
                    input = attributeNode.getChildren().toArray(new AttributeNode[0]);
                } else {
     ...
     */
    private static class ReturnValueLabelProvider extends LabelProvider implements ITableLabelProvider {
