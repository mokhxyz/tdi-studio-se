...
public class ThreadDumpSaxEventHandler extends DefaultHandler {
    ...
    public void startElement(String uri, String localName, String name, Attributes attributes) throws SAXException {
        ...
        if ("thread".equals(name)) { //$NON-NLS-1$
            ...
            currentlyParsedThread = new ThreadElement(threadName, State.valueOf(threadState), Long.parseLong(blockedTime),
                    Long.parseLong(blockedCount), Long.parseLong(waitedTime), Long.parseLong(waitedCount), lockName,
                    lockOwnerName, Boolean.parseBoolean(isSuspended), Boolean.parseBoolean(isDeadlocked),
        ...
        if ("frame".equals(name)) { //$NON-NLS-1$
            ...
            StackTraceElement element = new StackTraceElement(className, methodName, fileName.equals("null") ? null : fileName,//$NON-NLS-1$
    ...
    public void endElement(String uri, String localName, String name) throws SAXException {
        ...
        // thread
        if ("thread".equals(name)) { //$NON-NLS-1$
            currentlyParsedThread.setStackTrace(stackTraceElements.toArray(new StackTraceElement[0]));
            threadListElements.add((IThreadElement) currentlyParsedThread);
        }
    }
