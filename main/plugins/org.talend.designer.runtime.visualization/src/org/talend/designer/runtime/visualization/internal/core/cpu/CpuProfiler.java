...
import org.talend.designer.runtime.visualization.internal.core.AbstractJvm;
...
import org.talend.designer.runtime.visualization.internal.core.MBeanServer;
import org.talend.designer.runtime.visualization.internal.core.Messages;
import org.talend.designer.runtime.visualization.internal.core.Snapshot;
...
public class CpuProfiler implements ICpuProfiler {
        ...
        if (type == ProfilerType.BCI) {
            ...
            while (true) {
                if (monitor.isCanceled()) {
                    invokeCpuProfilerMXBeanMethod(INTERRUPT_TRANSFORM, null, null);
    ...
    public void resume() throws JvmCoreException {
        if (type == ProfilerType.BCI) {
            ...
            if (objectName != null) {
                jvm.getMBeanServer().setAttribute(objectName, new Attribute(RUNNING, true));
            }
        } else {
    ...
    public IFileStore dump() throws JvmCoreException {
        String dump = cpuModel.getCpuDumpString(jvm.getPid() + "@" //$NON-NLS-1$
                + jvm.getHost().getName(), jvm.getMainClass(), jvm.getMBeanServer().getJvmArguments());
        ...
        try {
            ...
            throw new JvmCoreException(IStatus.ERROR,
                    NLS.bind(Messages.dumpCpuProfileDataFailedMsg, fileStore.toURI().getPath()), e);
        } finally {
            if (os != null) {
                try {
                    os.close();
    ...
    public void setProfiledPackages(Set<String> packages) throws JvmCoreException {
        ...
        if (type == ProfilerType.BCI) {
            ...
            invokeCpuProfilerMXBeanMethod(SET_FILTER, new String[] { PROFILED_PACKAGES_PROP_KEY, buffer.toString() },
    ...
    private boolean isValidAgentVersion() throws JvmCoreException {
        if (agentJarVersion == null) {
            ObjectName objectName = jvm.getMBeanServer().getObjectName(PROFILER_MXBEAN_NAME);
            if (objectName != null) {
                Object value = jvm.getMBeanServer().getAttribute(objectName, VERSION);
                if (value == null) {
                    return false;
                }
    ...
    private boolean isBciProfilerRunning() throws JvmCoreException {
        ObjectName objectName = jvm.getMBeanServer().getObjectName(PROFILER_MXBEAN_NAME);
        if (objectName != null) {
            Object attribute = jvm.getMBeanServer().getAttribute(objectName, RUNNING);
     ...
     * @param params The parameters of method
     * @param signatures The signatures of method
    ...
    private Object invokeCpuProfilerMXBeanMethod(String method, String[] params, String[] signatures) throws JvmCoreException {
        ObjectName objectName = jvm.getMBeanServer().getObjectName(PROFILER_MXBEAN_NAME);
        return jvm.getMBeanServer().invoke(objectName, method, params, signatures);
    }
}
