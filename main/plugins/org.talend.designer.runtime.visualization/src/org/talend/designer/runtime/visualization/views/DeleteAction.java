...
public class DeleteAction extends Action implements ISelectionChangedListener {
    ...
    public void selectionChanged(SelectionChangedEvent event) {
        ...
        setEnabled(jvms.size() > 0 || snapshots.size() > 0 || remoteHosts.size() > 0);
    }
    ...
    private boolean confirm() {
        ...
        if (size == 1) {
            ...
            if (elementName.isEmpty()) {
                ...
                message = NLS.bind(Messages.confirmDeleteElementMsg, elementName);
            }
        } else {
            message = NLS.bind(Messages.confirmDeleteElementsMsg, String.valueOf(size));
        }

        return MessageDialog.openConfirm(treeViewer.getControl().getShell(), Messages.confirmDeleteTitle, message);
    }
