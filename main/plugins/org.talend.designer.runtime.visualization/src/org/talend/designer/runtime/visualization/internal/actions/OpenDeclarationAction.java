...
public class OpenDeclarationAction extends Action implements ISelectionChangedListener {
    ...
    public void run() {
        ...
        if (source == null) {
            MessageDialog.openError(PlatformUI.getWorkbench().getActiveWorkbenchWindow().getShell(), Messages.errorLabel,
                    Messages.openDeclarationFailedMsg);
            return;
        }
    ...
    private void parseSelection(IStructuredSelection selection) {
        ...
        } else if (element instanceof IHeapElement && !((IHeapElement) element).getClassName().endsWith("[]")) { //$NON-NLS-1$
    ...
    private IType searchSource() throws InterruptedException {
        final ProgressMonitorDialog dialog = new ProgressMonitorDialog(PlatformUI.getWorkbench().getActiveWorkbenchWindow()
                .getShell());
     ...
     */
    private static IEditorPart openEditor(IEditorInput editorInput, ISourcePresentation presentation, Object source)
            throws PartInitException {
        if (editorInput == null) {
            return null;
        }
     ...
     * @param level The inner class level
     * @param sourceRanges The candidates of source range
     * @throws JavaModelException
     */
     ...
     * 
     * @param signatures The type signatures e.g. <tt>QString;I</tt>
    ...
    /**
     * Gets the simplified parameters with given qualified parameters e.g. <tt>(java.lang.String, int)</tt>.
     * 
     * @param parameter The simplified parameters e.g. <tt>String, int</tt>
