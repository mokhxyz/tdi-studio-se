...
import org.talend.designer.runtime.visualization.internal.core.cpu.IThreadNode;
...
public class HotSpotsLabelProvider extends AbstractLabelProvider {
     ...
     * The constructor.
     * 
     * @param filteredTree The filteredTree
     */
    ...
    private static String getMethodColumnText(Object element) {
        if (element instanceof IThreadNode) {
            if (((IThreadNode) element).hasChildren()) {
                return Messages.threadLabel + ' ' + ((IThreadNode) element).getName();
            }
        } else if (element instanceof IMethodNode) {
     ...
     * 
     * @param column The hot spots column
     * @return The column index
     */
    private int getColumnIndex(HotSpotsColumn column) {
        Tree tree = filteredTree.getViewer().getTree();
        String label = column == HotSpotsColumn.HOT_SPOT ? filteredTree.getMethodColumnName() : column.label;
