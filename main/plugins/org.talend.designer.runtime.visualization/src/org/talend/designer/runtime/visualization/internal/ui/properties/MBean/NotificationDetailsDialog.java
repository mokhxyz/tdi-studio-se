...
import org.talend.designer.runtime.visualization.internal.ui.properties.PropertiesColumn;
...
class NotificationDetailsDialog extends Dialog {
     ...
     * The constructor.
     * 
     * @param tree The notification tree
     */
    ...
    protected void configureShell(Shell newShell) {
        ...
        PlatformUI.getWorkbench().getHelpSystem().setHelp(newShell, IHelpContextIds.NOTIFICATION_DETAILS_DIALOG);
    }
    ...
    protected void refreshWidgets() {
        final Notification notification = (Notification) ((StructuredSelection) tree.getViewer().getSelection())
                .getFirstElement();
    ...
    private void createTexts(Composite composite) {
        ...
        sequenceNumberText.setLayoutData(new GridData(GridData.FILL_HORIZONTAL));
        ...
        messageText = new Text(composite, SWT.BORDER | SWT.MULTI | SWT.V_SCROLL | SWT.WRAP);
    ...
    private Image getNextImage() {
        if (nextImage == null || nextImage.isDisposed()) {
            nextImage = Activator.getImageDescriptor(ISharedImages.NEXT_IMG_PATH).createImage();
        }
        return nextImage;
    }
    ...
    private Image getPrevImage() {
        if (prevImage == null || prevImage.isDisposed()) {
            prevImage = Activator.getImageDescriptor(ISharedImages.PREV_IMG_PATH).createImage();
        }
        return prevImage;
    }
    ...
    private class DetailsContentProvider implements ITreeContentProvider {
        ...
        public Object[] getElements(Object input) {
            if (attributeRootNode != null) {
                if (attributeRootNode.hasChildren()) {
                    return attributeRootNode.getChildren().toArray(new AttributeNode[0]);
                }
        ...
        public void refresh(Object userData) {
            if (attributeRootNode != null && attributeRootNode.getName().equals(userData.getClass().getCanonicalName())) {
                attributeRootNode.setValue(userData);
            } else {
                attributeRootNode = new AttributeNode(userData.getClass().getCanonicalName(), null, userData);
            }
     ...
     */
    private static class DetailsLabelProvider extends LabelProvider implements ITableLabelProvider {
