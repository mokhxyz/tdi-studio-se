...
import org.talend.designer.runtime.visualization.internal.ui.properties.MBean.MBeanLabelProvider;
...
public class AttributeLabelProvider extends MBeanLabelProvider {
    ...
    private Image getAttributeImage() {
        if (attributeImage == null || attributeImage.isDisposed()) {
            attributeImage = Activator.getImageDescriptor(ISharedImages.ATTRIBUTE_IMG_PATH).createImage();
        }
    ...
    private Image getAttributeFolderImage() {
        if (attributeFolderImage == null || attributeFolderImage.isDisposed()) {
            attributeFolderImage = Activator.getImageDescriptor(ISharedImages.ATTRIBUTE_FOLDER_IMG_PATH).createImage();
        }
