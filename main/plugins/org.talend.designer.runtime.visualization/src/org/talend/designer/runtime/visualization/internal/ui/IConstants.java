...
public interface IConstants {
    ...
    static final String PACKAGES_KEY = Activator.getDefault().getBundle().getSymbolicName() + ".packages";

    /** The dialog settings key for profiler sampling period. */
    static final String PROFILER_SAMPLING_PERIOD_KEY = Activator.getDefault().getBundle().getSymbolicName()
            + ".profilerSampingPeriod";

    /** The dialog settings key for profiler type (BCI or sampling). */
    static final String PROFILER_TYPE_KEY = Activator.getDefault().getBundle().getSymbolicName() + ".profilerType";
}
