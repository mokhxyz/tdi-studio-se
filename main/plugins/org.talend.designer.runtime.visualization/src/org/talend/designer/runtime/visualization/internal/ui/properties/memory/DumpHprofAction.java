...
public class DumpHprofAction extends Action {
    ...
    public void run() {
        ...
        try {
            if (jvm.isRemote()) {
                final FileNameInputDialog dialog = new FileNameInputDialog(section.getPart().getSite().getShell(),
     ...
     * @param transfer <tt>true</tt> to transfer file to local host
     */
    ...
    String getInitialFileName(IActiveJvm jvm) throws JvmCoreException {
        ...
        TabularData initialName = (TabularData) jvm.getMBeanServer().getAttribute(objectName, "SystemProperties"); //$NON-NLS-1$
    ...
    private static class FileNameInputDialog extends Dialog {
        ...
        public FileNameInputDialog(Shell shell, String initialFileName, boolean isAgentLoaded) {
            ...
            isFileTransfered = Activator.getDefault().getDialogSettings(getClass().getName()).getBoolean(TRANSFER_KEY);
        }
        ...
        protected void okPressed() {
            Activator.getDefault().getDialogSettings(getClass().getName()).put(TRANSFER_KEY, isFileTransfered);
            super.okPressed();
        }
