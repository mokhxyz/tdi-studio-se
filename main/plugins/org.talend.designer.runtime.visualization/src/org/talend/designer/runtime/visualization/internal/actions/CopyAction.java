...
public class CopyAction extends Action implements ISelectionChangedListener {
    ...
    /**
     * Creates copy action. If copy action is found in the given action bars, the found action will be returned,
     * otherwise copy action will be newly created and set to the given action bars as a global action.
     * 
    ...
    public void selectionChanged(SelectionChangedEvent event) {
        ...
        filesData.clear();
        for (Object element : ((StructuredSelection) event.getSelection()).toArray()) {
            if (buffer.length() > 0) {
                buffer.append('\n');
            }
