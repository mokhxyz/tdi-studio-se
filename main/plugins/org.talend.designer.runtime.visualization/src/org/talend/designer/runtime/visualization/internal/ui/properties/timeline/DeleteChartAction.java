...
public class DeleteChartAction extends Action {
    ...
    public void run() {
        ...
        if (MessageDialog.openConfirm(chart.getShell(), Messages.confirmDeleteChartTitle,
                NLS.bind(Messages.confirmDeleteChartMsg, chart.getSection().getText()))) {
            jvm.getMBeanServer().removeMonitoredAttributeGroup(chart.getAttributeGroup().getName());
        }
    }
}
