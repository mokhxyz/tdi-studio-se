    ...
    private void refresh() {
        new Job(Messages.refreshSubscribeActionStateJobLabel) {
            ...
            protected IStatus run(IProgressMonitor monitor) {
                ...
                boolean enabled = jvm != null && jvm.isConnected() && objectName != null
                        && jvm.getMBeanServer().getMBeanNotification().isSupported(objectName);
                setEnabled(enabled);
                if (enabled && jvm != null) {
                    setChecked(jvm.getMBeanServer().getMBeanNotification().isSubscribed(objectName));
                }
    ...
    public void run() {
        new Job(Messages.subscribeNotificationJobLabel) {
            ...
            protected IStatus run(IProgressMonitor monitor) {
                ...
                try {
                    if (jvm.getMBeanServer().getMBeanNotification().isSubscribed(objectName)) {
                        jvm.getMBeanServer().getMBeanNotification().unsubscribe(objectName);
                    } else {
                        jvm.getMBeanServer().getMBeanNotification().subscribe(objectName);
                    }
