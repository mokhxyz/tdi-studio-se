...
import org.talend.designer.runtime.visualization.agent.asm.Label;
import org.talend.designer.runtime.visualization.agent.asm.MethodAdapter;
import org.talend.designer.runtime.visualization.agent.asm.MethodVisitor;
import org.talend.designer.runtime.visualization.agent.asm.Opcodes;
...
public class MethodVisitorImpl extends MethodAdapter {
    ...
    public void visitCode() {
        instrumentMethodInvocation(Constants.METHOD_STEP_INTO, className, methodName);
    ...
    public void visitInsn(int opcode) {
        ...
        if ((Opcodes.IRETURN <= opcode && opcode <= Opcodes.RETURN) || Opcodes.ATHROW == opcode) {
            instrumentMethodInvocation(Constants.METHOD_STEP_RETURN, className, methodName);
        }
    ...
    public void visitLabel(Label label) {
        ...
        if (label.info != null) {
            instrumentMethodInvocation(Constants.METHOD_DROP_TO_FRAME, className, methodName, (String) label.info);
        }
    }
    ...
    private void instrumentMethodInvocation(String name, String... args) {
        ...
        super.visitMethodInsn(Opcodes.INVOKESTATIC, Constants.CLASS_CPU_PROFILER, name, desc);
    }
}
