...
import org.talend.designer.runtime.visualization.internal.core.cpu.CpuModelEvent;
...
import org.talend.designer.runtime.visualization.internal.ui.properties.cpu.actions.ClearCpuProfilingDataAction;
import org.talend.designer.runtime.visualization.internal.ui.properties.cpu.actions.DumpCpuProfilingDataAction;
import org.talend.designer.runtime.visualization.internal.ui.properties.cpu.actions.FindAction;
import org.talend.designer.runtime.visualization.internal.ui.properties.cpu.actions.FindAction.IFindTarget;
import org.talend.designer.runtime.visualization.internal.ui.properties.cpu.actions.ResumeCpuProfilingAction;
import org.talend.designer.runtime.visualization.internal.ui.properties.cpu.actions.SuspendCpuProfilingAction;

/**
 * The CPU section.
 */
public class CpuSection extends AbstractJvmPropertySection implements IFindTarget {
    ...
    public CpuSection() {
        ...
        resumeCpuProfilingAction.setSuspendCpuProfilingAction(suspendCpuProfilingAction);
        suspendCpuProfilingAction.setResumeCpuProfilingAction(resumeCpuProfilingAction);
    ...
    protected void setInput(IWorkbenchPart part, ISelection selection, IActiveJvm newJvm, IActiveJvm oldJvm) {
        ...
        if (oldJvm != null) {
            oldJvm.getCpuProfiler().getCpuModel().removeModelChangeListener(cpuModelChangeListener);
        }
     ...
     * 
     * @param initConfig <tt>true</tt> to initialize configuration.
     */
    private void refresh(final boolean initConfig) {
        new RefreshJob(NLS.bind(Messages.refeshCpuSectionJobLabel, getJvm().getPid()), toString()) {
            ...
            protected void refreshUI() {
                ...
                suspendCpuProfilingAction.setEnabled(isCpuProfilerRunning && isCpuProfilerReady && isConnected);
                resumeCpuProfilingAction.setEnabled(!isCpuProfilerRunning && isCpuProfilerReady && isPackageSpecified
                        && isConnected);
                clearCpuProfilingDataAction.setEnabled(isCpuProfilerReady && isPackageSpecified && isConnected);
     ...
     * Update the page.
     * 
     * @param isPackageSpecified True if packages are specified
     */
    void updatePage(boolean isPackageSpecified) {
        if (!isDisposed() && !callTree.isDisposed() && !hotSpots.isDisposed() && !callerCallee.isDisposed()) {
     ...
     */
    boolean isDisposed() {
        return callTree == null || hotSpots == null || callerCallee == null || callTree.isDisposed() || hotSpots.isDisposed()
    ...
    void setProfilerSamplingPeriod() {
        ...
        Integer period = null;
        String periodString = Activator.getDefault().getDialogSettings(CpuSection.class.getName())
    ...
    void setProfilerType() {
        ...
        String typeString = Activator.getDefault().getDialogSettings(CpuSection.class.getName())
    ...
    boolean isCpuProfilerReady() {
        ...
        return jvm != null
                && jvm.isConnected()
                && (jvm.getCpuProfiler().getState() == ProfilerState.READY || jvm.getCpuProfiler().getState() == ProfilerState.RUNNING);
    }
    ...
    boolean isCpuProfilerRunning() {
        IActiveJvm jvm = getJvm();
        return jvm != null && jvm.isConnected() && jvm.getCpuProfiler().getState() == ProfilerState.RUNNING;
    }
    ...
    boolean isPackageSpecified() {
        IActiveJvm jvm = getJvm();
        if (jvm == null || jvm.getCpuProfiler().getState() == ProfilerState.AGENT_NOT_LOADED) {
            return false;
        }
