...
public class NewJvmConnectionWizardPage extends WizardPage {
    ...
    private static final String REMOTE_HOST_HISTORY_KEY = Activator.getDefault().getBundle().getBundleId() + ".remoteHost"; //$NON-NLS-1$

    /** The dialog settings key for port history. */
    private static final String PORT_HISTORY_KEY = Activator.getDefault().getBundle().getBundleId() + ".port"; //$NON-NLS-1$

    /** The dialog settings key for user name history. */
    private static final String USER_NAME_HISTORY_KEY = Activator.getDefault().getBundle().getBundleId() + ".userName"; //$NON-NLS-1$

    /** The dialog settings key for URL history. */
    private static final String URL_HISTORY_KEY = Activator.getDefault().getBundle().getBundleId() + ".url"; //$NON-NLS-1$
    ...
    public NewJvmConnectionWizardPage(ISelection selection) {
        ...
        ImageDescriptor image = Activator.getImageDescriptor(ISharedImages.NEW_JVM_CONNECTION_IMG_PATH);
    ...
    public void createControl(Composite parent) {
        ...
        PlatformUI.getWorkbench().getHelpSystem().setHelp(composite, IHelpContextIds.NEW_JVM_CONNECTION_WIZARD_PAGE);
    }
     ...
     * 
     * @return The state indicating if the host and port radio button is selected
     */
    ...
    private void createAuthenticateGroup(Composite parent) {
        ...
        userNameText = addComboTextField(group, Messages.userNameTextLabel, USER_NAME_HISTORY_KEY);
    ...
    private void createUrlPanel(Composite parent) {
        ...
        urlText = addComboTextField(parent, Messages.jmxUrlTextLabel, URL_HISTORY_KEY);
        urlText.setEnabled(false);
    }
     ...
     * 
     * @param key The dialog settings key
