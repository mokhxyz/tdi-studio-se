...
public interface IMBeanServer {
     ...
     * Queries the names.
     * 
     * @param objectName The object name, or <tt>null</tt> to get all names
     ...
     * @throws JvmCoreException
     */
    Object getAttribute(ObjectName objectName, String attributeName) throws JvmCoreException;
     ...
     * @throws JvmCoreException
     */
    void setAttribute(ObjectName objectName, Attribute attribute) throws JvmCoreException;
     ...
     */
    IMonitoredMXBeanGroup addMonitoredAttributeGroup(String name, AxisUnit axisUnit);

    /**
     * Removes the monitored attribute group.
     * 
     * @param name The monitored attribute group name
     */
     ...
     * @param params the parameters, or <tt>null</tt> if having no arguments
     * @param signatures The signatures, or <tt>null</tt> if having no arguments
     * @return The return value, or <tt>null</tt> if not connected
     * @throws JvmCoreException
     */
    Object invoke(ObjectName objectName, String method, Object[] params, String[] signatures) throws JvmCoreException;
    ...
    /**
     * Gets the thread cache in JVM model without accessing to target JVM. To get the up-to-dated thread data,
     * {@link #refreshThreadCache()} has to be invoked.
     * 
    ...
    /**
     * Gets the heap cache in JVM model without accessing to target JVM. To get the up-to-dated heap data,
     * {@link #refreshHeapCache()} has to be invoked.
     * 
     ...
     * 
     * @param hprofFileName The hprof file name used when monitoring JVM on remote host
     * @param transfer <tt>true</tt> to transfer hprof file to local host
     * @param monitor The progress monitor
     * @return The file store, or <tt>null</tt> if target JVM is running on remote host and hprof file is not transfered
     * to local host
     * @throws JvmCoreException
     */
    IFileStore dumpHprof(String hprofFileName, boolean transfer, IProgressMonitor monitor) throws JvmCoreException;
     ...
     * 
     * @param listener The MBean server change listener
     */
