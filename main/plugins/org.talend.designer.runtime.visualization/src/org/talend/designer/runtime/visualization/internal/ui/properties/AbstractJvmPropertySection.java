 ...
 */
abstract public class AbstractJvmPropertySection extends AbstractPropertySection implements IJvmModelChangeListener {
    ...
    public void jvmModelChanged(final JvmModelEvent e) {
        ...
        if (pageBook.isDisposed() || contentPage.isDisposed() || messagePage.isDisposed() || messageLabel.isDisposed()
     ...
     * 
     * @param suspend True if suspending refresh
     */
     ...
     * Sets pinned.
     * 
     * @param pinned <tt>true</tt> to set pinned
     */
    ...
    protected void updatePage() {
        if (jvm == null || pageBook.isDisposed() || messagePage.isDisposed() || messageLabel.isDisposed()) {
            return;
        }
     ...
     */
    abstract protected void setInput(IWorkbenchPart part, ISelection selection, IActiveJvm newJvm, IActiveJvm oldJvm);
    ...
    private void addLocalMenus() {
        ...
        Display.getDefault().asyncExec(new Runnable() {
            ...
            public void run() {
                IMenuManager menuManager = getMenuManager();
                if (menuManager != null && menuManager.find(preferencesAction.getId()) == null) {
